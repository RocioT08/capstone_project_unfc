...
def error_metrics(actuals: np.ndarray, preds: np.ndarray) -> Dict[str, float]:
    """MSE, RMSE, MAE, MAPE plus R² sum-of-squares components (ss_res, ss_tot)
    so pooled R² can be computed exactly across windows: 1 − Σss_res/Σss_tot."""
    ...
    return {
        ...
        "mape": float(np.mean(np.abs(err) / np.abs(a)) * 100),
